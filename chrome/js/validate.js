...
export function addCommentValidation() {
    ...
    const observer = new MutationObserver(async () => {
        ...
        if (commentWriter && registerButton) {
            ...
            // 크롬에서는 파이어폭스와 달리, 부모 요소에서 클릭 이벤트를 가로채더라도
            // 자식 요소의 href 속성이 작동합니다.
            // 해당 문제를 해결하고자 댓글 등록 버튼의 href 속성을 제거했고,
            // href 속성을 제거할 경우 마우스 커서가 text 타입으로 표시되어
            // 스타일에서 마우스 커서를 pointer 타입으로 표시하도록 명시해 주었습니다.
